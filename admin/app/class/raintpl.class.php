	...
	private function path_replace( $html, $tpl_basedir ){
		...
		if( self::$path_replace ){
			...
			if( in_array( "img", self::$path_replace_list ) ){
				...
				$sub = array( '<img$1src=@$2://$3@', '<img$1src=@$2@', '<img$1src="' . self::$base_url . '$2"', '<img$1src="$2"' );
			}
			...
			if( in_array( "script", self::$path_replace_list ) ){
				...
				$sub = array_merge( $sub , array( '<script$1src=@$2://$3@', '<script$1src=@$2@', '<script$1src="' . self::$base_url . '$2"', '<script$1src="$2"' ) );
			}
			...
			if( in_array( "link", self::$path_replace_list ) ){
				...
				$sub = array_merge( $sub , array( '<link$1href=@$2://$3@', '<link$1href=@$2@' , '<link$1href="' . self::$base_url . '$2"', '<link$1href="$2"' ) );
			}

			if( in_array( "a", self::$path_replace_list ) ){
				$exp = array_merge( $exp , array( '/<a(.*?)href=(?:")(mailto)\:([^"]+?)(?:")/i', '/<a(.*?)href=(?:")(http|https)\:\/\/([^"]+?)(?:")/i', '/<a(.*?)href="(.*?)"/', '/<a(.*?)href=(?:\%)([^"]+?)(?:\%)/i'  ) );
				$sub = array_merge( $sub , array( '<a$1href=%$2:$3%', '<a$1href=%$2://$3%', '<a$1href="' . self::$base_url . '$2"', '<a$1href="$2"' ) );
			}
			...
			if( in_array( "input", self::$path_replace_list ) ){
				...
				$sub = array_merge( $sub , array( '<input$1src=@$2://$3@', '<input$1src=@$2@', '<input$1src="' . self::$base_url . '$2"', '<input$1src="$2"' ) );
			}

			if( in_array( "form", self::$path_replace_list ) ){
				$exp = array_merge( $exp , array( '/<form(.*?)action=(?:")(http|https)\:\/\/([^"]+?)(?:")/i', '/<form(.*?)action=(?:")([^"]+?)#(?:")/i', '/<form(.*?)action="(.*?)"/', '/<form(.*?)action=(?:\@)([^"]+?)(?:\@)/i' ) );
