...
foreach ($row2 as $row) {
	...
	$sqlInsert="INSERT INTO miembros (id, fecha_alta, cuota, pago, nombre, email, web, id_presidente, id_vicepresidente, id_secretario, id_tesorero, direccion, cp, localidad, id_provincia, telefono, telefono1, telefono2, movil, fax, observaciones) VALUES	(".$row['ID'].", '".$fecha_alta."', ".$row['CUOTAANUAL'].", ".$row['CUOTAPAGADA'].", '".utf8_encode(mysql_real_escape_string($row['NOMBRE']))."', '".$email."','".$web."', ".$row['IDPRESIDENTE'].", ".$row['IDVICEPRESIDENTE'].", ".$row['IDSECRETARIO'].", ".$row['IDTESORERO'].", '".utf8_encode(mysql_real_escape_string($row['DIR1']))."', '".$row['CP1']."', '".utf8_encode(mysql_real_escape_string($row['LOCALIDAD1']))."', ".$provincia.", '".$row['TEL']."', '".$row['TEL2']."', '".$row['TEL3']."', '".$row['MOVIL']."', '".$row['FAX']."', '".utf8_encode(mysql_real_escape_string($row['OBSERVACIONES']))."')";
	//echo $sqlInsert;
